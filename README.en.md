...
> **Note (`username` flow):** requires all `dms`/`myPeople` environment variables listed above, plus `DMLCONFIGSYNC_DB_*` (`woc.dms_settings`/`woc.dml_configurations` cache) and `DBMANAGER_DB_*` (`woc.ang_snowflakes` table, for `marketIso`) — see `dmlConfigSync`/`dbManager` sections below (same Aurora cluster/secret as `pkFavorite`, no new CloudFormation parameter).
