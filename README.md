...
| `oics` | array `Response.User.OICs` di myPeople **filtrato** (tolti gli OIC con `enablewoc = 0` in `woc.hq_application_enabling`, vedi nota sopra), con tutte le chiavi di ogni oggetto in minuscolo (es. `MARKET`→`market`, `CODE`→`code`, `BRANDS`→`brands`, ...); il campo `brands` è sovrascritto con l'elenco letto da `woc.ang_snowflakes` (fallback al CSV di myPeople, vedi nota sopra) e ogni elemento include inoltre `brandLogos` subito dopo `brands` (vedi nota sopra) |
