...
> ℹ️ **Cache DynamoDB per-VIN**: sia l'azione `getdetails` (Lambda `handler`) sia il CLI usano `getDetailsWithCache` (`v360Service.js`) invece di chiamare direttamente `getDetails`. Prima di interrogare ASV360, si legge la cache condivisa DynamoDB (`TmpCacheTable`, chiave `v360:getdetails:<vin>`, TTL 1h configurabile via `SESSION_CACHE_TTL_SECONDS`, v. `dynamoCache.js`): se presente e non scaduta (scritta da una precedente chiamata riuscita per lo stesso VIN, condivisibile fra tutte le istanze/funzioni Lambda, a differenza della precedente cache su file `/tmp` locale all'istanza), la risposta viene restituita senza rifare la chiamata; altrimenti si chiama ASV360 e la risposta viene salvata in DynamoDB per le richieste successive entro il TTL. Il salvataggio è **best-effort**: un eventuale errore di scrittura viene solo loggato come warning e non fa fallire la richiesta (stesso pattern di `jobcard/jobCardService.js::saveJobCardDetailsToTmp`). La stessa cache è usata anche da `getCachedBrand(vin)`, funzione best-effort usata dai chiamanti esterni (`jobcard`, `pkFavorite`, `pkManager`) per risolvere `data.brandCode` senza doverlo ricevere dal FE — v. `dms/dmsService.js::resolveDynamicSenderFields`.
